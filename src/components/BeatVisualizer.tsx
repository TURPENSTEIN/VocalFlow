...
export const BeatVisualizer: React.FC<BeatVisualizerProps> = ({
  ...
  // Cycle progress: 4 bars * 16 steps = 64 total steps in a 4-bar cycle
  ...
  return (
    <div id="beat-visualizer" className="w-full bg-[#0a0f0d] border border-[#1b3323] rounded-xl p-3 sm:p-4 font-mono">
      {/* Top Header: Bar Counter & Rhythm Meter */}
      <div className="flex flex-wrap items-center justify-between gap-3 mb-2.5 text-xs border-b border-[#16271c] pb-2">
        <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
          <span className="text-zinc-500 uppercase tracking-widest text-[10px] sm:text-[11px]">BAR CYCLE:</span>
          <div className="flex items-center gap-1 sm:gap-1.5">
            {[0, 1, 2, 3].map((barIdx) => {
              ...
              return (
                <div
                  key={barIdx}
                  className={`px-2 sm:px-2.5 py-1 text-[11px] sm:text-xs font-bold rounded border transition-all duration-100 ${
                    isActive
        ...
        <div className="flex items-center gap-3 sm:gap-4">
          <div className="text-right">
            <span className="text-zinc-500 text-[10px] block uppercase">Next Prompt Flash</span>
            <span className="text-[#00e5ff] font-bold text-xs sm:text-sm">
              {isPlaying
                ? (barsRemaining === 0 ? 'FINAL BAR (FLASH NEXT)' : `IN ${barsRemaining} BAR${barsRemaining === 1 ? '' : 'S'}`)
                : 'STANDBY'}
            </span>
          </div>
          ...
          <div className="border-l border-zinc-800 pl-3">
            ...
            <span className="text-zinc-200 font-bold text-xs sm:text-sm">{isPlaying ? totalBars : 0}</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-4 gap-1.5 sm:gap-2 mb-2.5">
        {[0, 1, 2, 3].map((beatIdx) => {
          ...
          return (
            <div
              key={beatIdx}
              className={`flex flex-col items-center justify-center py-2 sm:py-2.5 px-1 rounded border transition-all duration-75 ${
                isBeatActive
                  ? isDownbeat
            ...
            >
              <div className="flex items-center gap-1.5">
                <span
                  className={`w-2 sm:w-2.5 h-2 sm:h-2.5 rounded-full transition-transform duration-75 ${
      ...
      <div className="w-full bg-[#050806] rounded border border-[#14231a] p-1.5">
        <div className="flex justify-between items-center text-[10px] text-zinc-500 mb-1 px-1">
          <span>4-BAR CYCLE COUNTDOWN</span>
          <span className="text-[#00ff88] font-bold">{Math.round(cycleProgressPercent)}%</span>
        </div>
        <div className="w-full bg-zinc-900/90 h-2 rounded overflow-hidden relative">
          <div
