...
export const AudioOscilloscope: React.FC<AudioOscilloscopeProps> = ({
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      if (waveData && waveData.length > 0) {
        ...
        phase += 0.06;
        const beatAmplitude = isBeatPlaying ? (activeBeat === 0 ? 20 : 12) : 4;

        ctx.lineWidth = 1.8;
        ctx.strokeStyle = isBeatPlaying ? '#00e5ff' : '#27382d';
  ...
  return (
    <div id="audio-oscilloscope-container" className="relative w-full rounded-lg border border-[#1a2e22] bg-[#080c0a] overflow-hidden p-1 font-mono">
      <div className="flex items-center justify-between px-3 py-1 text-xs border-b border-[#14231a] text-[#00ff88]">
        <div className="flex items-center gap-2">
          ...
          <span className="tracking-wider font-semibold text-[11px] sm:text-xs">
            {isRecording ? 'MIC LIVE [RECORDING]' : (isBeatPlaying ? 'DRUM ENGINE [ACTIVE]' : 'OSCILLOSCOPE [STANDBY]')}
          </span>
        </div>
        <div className="text-zinc-500 text-[10px] uppercase tracking-widest hidden sm:block">
          {isRecording ? 'AUDIO INPUT: CAPTURING' : 'MONITOR POCKET'}
        </div>
      </div>
      <canvas
        ref={canvasRef}
        width={750}
        height={72}
        className="w-full h-[65px] sm:h-[72px] block"
      />
    </div>
  );
};
