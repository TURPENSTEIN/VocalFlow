...
export const SessionSummaryModal: React.FC<SessionSummaryModalProps> = ({
  ...
  const interferenceRating = analytics.distractionEnabled
    ? (analytics.distractionIntensity === 'high' ? 'EXPERT INTERFERENCE' : analytics.distractionIntensity === 'medium' ? 'ADVANCED INTERFERENCE' : 'MODERATE INTERFERENCE')
    : 'CLEAR (BASELINE)';
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/85 backdrop-blur-md overflow-y-auto">
      <div
        ...
        className="w-full max-w-2xl bg-[#080c09] border-2 border-[#1b3b28] rounded-xl p-4 sm:p-7 shadow-[0_0_50px_rgba(0,255,136,0.25)] text-zinc-200 font-mono relative my-auto max-h-[90vh] flex flex-col justify-between overflow-y-auto custom-scrollbar"
      >
        {/* Neon corner accents */}
        ...
        <div className="flex items-center justify-between border-b border-[#14261b] pb-3 mb-4">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-md bg-[#00ff88]/15 border border-[#00ff88]/40 flex items-center justify-center text-[#00ff88] shrink-0">
              <Trophy size={20} />
            </div>
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-lg sm:text-2xl font-extrabold text-white text-glow-green tracking-tight">
                  ROUND COMPLETE
                </h2>
                <span className="text-[10px] uppercase font-bold px-1.5 py-0.5 rounded bg-[#00e5ff]/20 text-[#00e5ff] border border-[#00e5ff]/40">
                  {analytics.trainingMode.toUpperCase()}
                </span>
              </div>
              <p className="text-[11px] text-zinc-400">
                VocalFlow Performance Telemetry & Speech Review
              </p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 mb-5">
          <div className="bg-[#0c130f] border border-[#16291e] rounded-lg p-3">
            ...
            <div className="text-[10px] text-zinc-500">4-Bar Cycles</div>
          </div>
          ...
          <div className="bg-[#0c130f] border border-[#16291e] rounded-lg p-3">
            ...
            <div className="text-2xl sm:text-3xl font-extrabold text-[#00e5ff]">
              {analytics.averageBpm}
            </div>
            <div className="text-[10px] text-zinc-500">Tempo Lock</div>
          </div>
          ...
          <div className="bg-[#0c130f] border border-[#16291e] rounded-lg p-3">
            ...
            <div className="text-[10px] text-zinc-500">Measures Delivered</div>
          </div>
          ...
          <div className="bg-[#0c130f] border border-[#16291e] rounded-lg p-3">
            ...
            <div className="text-sm sm:text-base font-extrabold text-purple-300 truncate">
              {interferenceRating}
            </div>
            <div className="text-[10px] text-zinc-500">Attention Load</div>
          </div>
        </div>

        {/* Recorded Vocal Review Audio Player */}
        <div className="bg-[#0c130f] border border-[#182f22] rounded-lg p-3.5 sm:p-4 mb-4">
          <div className="flex items-center justify-between mb-2.5 flex-wrap gap-2">
            <div className="flex items-center gap-2 text-xs font-bold text-[#00ff88] uppercase tracking-wider">
              <Volume2 size={15} />
              <span>RECORDED VOCAL PLAYBACK</span>
            </div>
            {analytics.audioUrl && (
              <a
                ...
                className="flex items-center gap-1.5 px-2.5 py-1 bg-[#00ff88]/15 border border-[#00ff88]/40 hover:bg-[#00ff88] hover:text-black text-[#00ff88] text-xs font-bold rounded transition-colors"
              >
                <Download size={13} />
                DOWNLOAD AUDIO (.WEBM)
              </a>
          ...
          {analytics.audioUrl ? (
            ...
            <div className="text-center py-3 text-xs text-zinc-400 bg-black/30 rounded border border-dashed border-zinc-800">
              No audio recorded. Tap the <span className="text-red-400 font-bold">RECORD VOCALS</span> toggle before or during your next round to record your live speech!
            </div>
          )}
        </div>
        ...
        <div className="mb-5 flex-1">
          <div className="text-xs font-bold text-zinc-400 uppercase tracking-wider mb-2 flex items-center gap-1.5">
            ...
            TRIALS DELIVERED THIS SESSION ({analytics.trials.length})
          </div>
          ...
          <div className="max-h-36 overflow-y-auto space-y-1.5 pr-1 custom-scrollbar">
            {analytics.trials.map((trial, idx) => (
              <div
                key={idx}
                className="flex items-center justify-between p-2 rounded bg-[#070c09] border border-[#142319] text-xs"
              >
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="text-[10px] text-zinc-500 font-bold w-4">#{idx + 1}</span>
                  <div className="flex items-center gap-1.5">
                    {trial.words.map((w, wIdx) => (
                      <span key={wIdx} className="font-extrabold text-[#00ff88] tracking-wider">
                        {w}
                      </span>
                    ))}
                  </div>
                  {trial.tabooWords && trial.tabooWords.length > 0 && (
                    <span className="text-[10px] text-red-400 border-l border-zinc-800 pl-2">
                      🚫 {trial.tabooWords.join(', ')}
                    </span>
                  )}
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 pt-3 border-t border-[#14261b]">
          <button
            ...
            title="Download complete standalone single-file HTML version"
          >
          ...
          <div className="flex items-center gap-2 ml-auto">
            ...
            <button
              ...
              onClick={onRestart}
              className="flex items-center gap-2 px-5 py-2.5 bg-[#00ff88] text-black font-extrabold text-xs sm:text-sm rounded-md hover:bg-[#1aff96] active:scale-95 transition-all shadow-[0_0_15px_rgba(0,255,136,0.3)] cursor-pointer"
            >
              <RotateCcw size={15} />
              START NEW ROUND
            </button>
