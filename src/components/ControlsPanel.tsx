...
import { DrumStyle, TrainingMode } from '../types';
...
interface ControlsPanelProps {
  ...
  onChangeTrainingMode: (mode: TrainingMode) => void;
  wordCount: number;
...
export const ControlsPanel: React.FC<ControlsPanelProps> = ({
  ...
  return (
    <div id="controls-panel" className="w-full bg-[#080c09] border border-[#1b3323] rounded-xl p-3 sm:p-5 font-mono flex flex-col gap-4">
      {/* 1. Mode Selector & Word Quantity Bar */}
      <div className="flex flex-wrap items-center justify-between gap-2.5 border-b border-[#14261b] pb-3">
        <div className="flex items-center gap-2 flex-wrap">
          <span className="text-[10px] sm:text-xs text-zinc-400 font-bold uppercase flex items-center gap-1">
            <Flame size={13} className="text-[#00ff88]" />
            TRAINING MODE:
          </span>
          <div className="flex items-center gap-1 flex-wrap">
            {[
              { id: 'classic', label: 'CLASSIC', desc: 'Word + Rhyme/Meter' },
              { id: 'taboo', label: '⛔ TABOO', desc: 'Forbidden Words' },
              { id: 'multi_word', label: 'MULTI-WORD', desc: 'Multiple Words' },
              { id: 'gauntlet', label: '⚡ GAUNTLET', desc: 'Word + Taboo + Meter' },
            ].map((m) => (
              <button
                key={m.id}
                onClick={() => onChangeTrainingMode(m.id as TrainingMode)}
                className={`px-2.5 py-1 text-[10px] sm:text-xs font-bold rounded border transition-all ${
                  trainingMode === m.id
                    ? m.id === 'taboo'
                      ? 'border-red-500 bg-red-950/60 text-red-300 shadow-[0_0_10px_rgba(255,51,85,0.4)]'
                      : 'border-[#00ff88] bg-[#00ff88]/20 text-[#00ff88] shadow-[0_0_10px_rgba(0,255,136,0.3)]'
        ...
        {/* Word Quantity Toggle (1, 2, 3 Words) */}
        <div className="flex items-center gap-2">
          <span className="text-[10px] sm:text-xs text-zinc-400 uppercase font-semibold flex items-center gap-1">
          ...
          <div className="flex items-center gap-1">
            {[1, 2, 3].map((count) => (
              <button
                ...
                title={`${count} mandatory word${count > 1 ? 's' : ''} per 4 bars`}
              >
                {count}W
              </button>
            ))}
      ...
      {/* 2. Primary Action Bar: Start / Stop, Mic Toggle, Round Clock */}
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[#14261b] pb-4">
        ...
        <div className="flex items-center gap-2.5 flex-wrap">
          {!isPlaying ? (
            <button
              id="btn-start-round"
              onClick={onStart}
              className="flex items-center gap-2 px-5 sm:px-6 py-2.5 sm:py-3 bg-[#00ff88] text-black font-extrabold text-sm sm:text-base rounded-md hover:bg-[#1aff96] active:scale-95 transition-all shadow-[0_0_20px_rgba(0,255,136,0.4)] cursor-pointer"
            ...
            <div className="flex items-center gap-2">
              <button
                id="btn-stop-round"
                onClick={onStop}
                className="flex items-center gap-2 px-4 sm:px-5 py-2.5 sm:py-3 bg-[#ff3355] text-white font-bold text-sm sm:text-base rounded-md hover:bg-[#ff4d6d] active:scale-95 transition-all shadow-[0_0_20px_rgba(255,51,85,0.4)] cursor-pointer"
              ...
              <button
                ...
                className="px-3 py-2.5 sm:py-3 border border-zinc-700 bg-zinc-900/90 text-zinc-300 hover:text-white hover:border-zinc-500 text-xs font-semibold rounded-md transition-colors"
          ...
          <button
            id="btn-toggle-mic"
            onClick={onToggleMic}
            className={`flex items-center gap-2 px-3.5 sm:px-4 py-2.5 sm:py-3 rounded-md font-semibold text-xs sm:text-sm border transition-all cursor-pointer ${
              isMicActive
          ...
          >
            ...
            <span>{isMicActive ? 'MIC RECORDING [ON]' : 'RECORD VOCALS [OFF]'}</span>
          </button>
        </div>

        {/* Round Timer Indicator */}
        <div className="flex items-center gap-3 bg-[#0c130f] px-3.5 py-2 rounded-md border border-[#16271c] ml-auto">
      ...
      {/* 3. BPM Slider, Drum Groove Style, Volume */}
        ...
        <div className="md:col-span-5 bg-[#050806] p-3 rounded-lg border border-[#142319]">
          <div className="flex items-center justify-between text-xs mb-1.5">
            ...
            <div className="flex items-center gap-1">
              ...
              <span className="text-base font-extrabold text-[#00ff88] px-1.5 min-w-[50px] text-center">
                {bpm}
              </span>
              <button
                id="btn-bpm-plus"
          ...
          <div className="grid grid-cols-4 gap-1">
            {(['boombap', 'trap', 'lofi', 'drill'] as DrumStyle[]).map((style) => (
              <button
                ...
                className={`py-1.5 text-[10px] sm:text-[11px] font-bold uppercase rounded border transition-colors ${
                  drumStyle === style
                    ? 'border-[#00e5ff] bg-[#00e5ff]/20 text-[#00e5ff]'
      ...
      {/* 4. Distraction Mode (Interference Control / Selective Attention) */}
      <div className="border-t border-[#14261b] pt-3 flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2.5 flex-wrap">
          <button
            ...
            title="Toggle distraction noise to train selective attention [D]"
          >
            ...
            <AlertTriangle size={14} className={distractionEnabled ? 'text-amber-400 animate-bounce' : 'text-zinc-500'} />
            <span>DISTRACTION MODE: {distractionEnabled ? 'ARMED [INTERFERENCE ON]' : 'OFF [CLEAR AUDIO]'}</span>
          </button>

          {distractionEnabled && (
            <div className="flex items-center gap-1 bg-[#120f07] p-1 rounded border border-amber-900/60">
              <span className="text-[10px] text-amber-500/80 px-1.5 uppercase font-bold hidden sm:inline">INTENSITY:</span>
        ...
        <div className="text-[10px] sm:text-[11px] text-zinc-500 font-mono italic">
          Cognitive Interference Training Engine
        </div>
      </div>
    </div>
  );
